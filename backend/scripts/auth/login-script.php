...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['btn_SignIn'])) {
    ...
    if (!$user) {
        echo "<script>window.location.href='" . ConfigClass::baseURL() . "frontend/auth/index.php?alert=user_not_found';</script>";
    } else {
        ...
        if (password_verify($password, $user['password_hashed'])) {
            ...
            if ($user['role'] === 'ADMIN' || $user['role'] === 'MANAGER' || $user['role'] === 'REQUESTOR' || $user['role'] === 'TECHNICIAN') {
                header('Location: ' . ConfigClass::baseURL() . 'frontend/views/dashboard/index.php');
            } else {
                header('Location: ' . ConfigClass::baseURL() . 'frontend/auth/index.php?alert=user_not_found');
            }
        } else {
            // Incorrect password, show an error message
            echo "<script>window.location.href='" . ConfigClass::baseURL() . "frontend/auth/index.php';</script>";
        }
    }
}